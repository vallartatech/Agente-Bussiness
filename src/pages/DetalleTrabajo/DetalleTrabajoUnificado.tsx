...
export interface DetalleTrabajoConfig {
    basePath: string;
    initialTabDefault: "Datos" | "Trabajo" | "Cotización" | "Registro" | "Historial";
    canCotizar: boolean;
    notificarEcosistema: boolean;
    // canAsignarTecnico, canCambiarEstado, etc. can be inferred from the user role in the component as before, 
    // or passed via config. For now, the main differences are basePath, initialTab, canCotizar, and notificarEcosistema.
}

const DetalleTrabajoUnificado: React.FC<{ config: DetalleTrabajoConfig }> = ({ config }) => {
    const { id } = useParams();
        ...
        const fetchAll = async () => {
            ...
            try {
                ...
                const mappedJob = {
                    ...
                    horaAsignada: data.horaAsignada ? (data.horaAsignada.length > 5 ? data.horaAsignada.substring(0, 5) : data.horaAsignada) : (data.hora_asignada ? (data.hora_asignada.length > 5 ? data.hora_asignada.substring(0, 5) : data.hora_asignada) : (data.hora_programada || null)),
                    hora_programada: data.horaAsignada ? (data.horaAsignada.length > 5 ? data.horaAsignada.substring(0, 5) : data.horaAsignada) : (data.hora_programada || data.hora_asignada || null),
                    ...
                    trabajador_id: data.trabajador_id || data.trabajador?.id || null,
                    trabajador: data.trabajador || null,
                    tecnicoUserId: data.trabajador?.user_id || null
                };
        ...
        const fetchActividades = async () => {
            ...
            try {
                ...
                const mappedSubTareas = acts.map((act: any) => {
                    ...
                    // Deduplicate corrupted DB data: remove items that exist in quoteData
                    if (finalRefacciones.length > 0 && qData) {
                        const qConceptNames = (qData.conceptos || []).map((c: any) => c.descripcion?.trim().toLowerCase());
                        const qMaterialNames = (qData.materiales || []).map((m: any) => m.nombre?.trim().toLowerCase());

                        finalRefacciones = finalRefacciones.filter((r: any) => {
                            const rName = r.pieza?.trim().toLowerCase() || '';
                            const rNameClean = rName.replace(' (material)', '').trim();

                            if (qConceptNames.includes(rNameClean)) return false;
                            if (qMaterialNames.includes(rNameClean)) return false;
    ...
    const [showSendQuoteConfirmModal, setShowSendQuoteConfirmModal] = useState(false);
    ...
    const handleSaveExecutionTime = async () => {
        ...
        try {
            if (trabajo?.id) {
                ...
                localStorage.setItem(`trabajo_updated_${trabajo.id}`, Date.now().toString());
    ...
    /** Técnico → Iniciar Ejecución del Trabajo */
    const handleIniciarEjecucionTrabajo = async () => {
        if (!trabajo?.id) return;
        try {
            if (trabajo.estado !== 'En Ejecución') {
                await updateTrabajo(trabajo.id, { estado: 'En Ejecución' });
                ...
                try {
                    if (trabajo.admin_autonomo_id) {
                        await createNotificacion({
                            user_id: trabajo.admin_autonomo_id,
                            titulo: '🚀 Ejecución de Trabajo Iniciada',
                            mensaje: `El técnico ${user?.name || trabajo.tecnico || ''} ha iniciado la ejecución del trabajo en "${trabajo.sucursal || 'la sucursal'}".`,
                            enlace: `/autonomo/trabajo-detalle/${trabajo.id}`
                        });
                    }
                    await createNotificacionByRole({
                ...
                showAlert(
                    ...
                    "Has iniciado formalmente la ejecución de este trabajo. El Administrador ha sido notificado.",
                    "success"
                );
            }

            if (subTareas && subTareas.length > 0) {
                const firstPending = subTareas.find(t => t.estado !== 'Completa' && !localStorage.getItem(`report_data_${t.id}`)) || subTareas[0];
                handleAbrirReporteTarea(firstPending.id);
            } else {
                handleAbrirReporteTarea();
            }
        } catch (err: any) {
            console.error("Error iniciando ejecución del trabajo:", err);
            showAlert("Error", "No se pudo actualizar el estado del trabajo a En Ejecución.", "error");
        }
    };
        ...
        try {
            ...
            if (trabajo.negocio_id) {
                ...
                if (negocioData) {
                    ...
                    if (config.notificarEcosistema && negocioData.admin_autonomo_id) {
                        try {
                            await createNotificacionEcosistema({
                                admin_autonomo_id: negocioData.admin_autonomo_id,
                                titulo: 'Técnico en camino',
    ...
    const executeSendQuote = async () => {
        ...
        try {
            ...
            if (config.notificarEcosistema && trabajo?.admin_autonomo_id) {
                await createNotificacionEcosistema({
            ...
            showAlert('Cotización Enviada', 'Se ha enviado la cotización al Admin Autónomo.', 'success');
            setShowSendQuoteConfirmModal(false);
            setQuoteToSend(null);
        } catch (error) {
            console.error("Error al enviar al admin autonomo:", error);
            showAlert('Error', 'Hubo un problema al enviar la cotización.', 'error');
        } finally {
    ...
    /** Admin General (Ecosistema Autónomo) → Reactivar la sugerencia de cotización del técnico que fue rechazada */
    const handleReactivarCotizacionSugerida = async () => {
        if (!trabajo) return;
        try {
            // 1. Actualizar el estado del trabajo a "Cotización Reactivada" en backend
            await updateEstadoTrabajo(trabajo.id, { estado: "Cotización Reactivada" });

            // 2. Si existen cotizaciones en la tabla con estado Rechazada, reactivarlas a Pendiente
            if (cotizaciones.length > 0) {
                for (const c of cotizaciones.filter(c => c.estado === 'Rechazada')) {
                    try {
                        await updateCotizacionStatus(c.id!, "Pendiente");
                    } catch (err) {
                        console.warn("Error reactivando cotización:", err);
                    }
                }
                setCotizaciones(prev => prev.map(c => c.estado === 'Rechazada' ? { ...c, estado: "Pendiente" as const } : c));
            }

            // 3. Actualizar subTareas locales para cambiar cotizacionEstado a Reactivada
            setSubTareas(prev => prev.map(t => ({
                ...t,
                cotizacionEstado: "Reactivada" as any
            })));

            // 4. Actualizar trabajo localmente con estado "Cotización Reactivada"
            setTrabajo(prev => prev ? { ...prev, estado: "Cotización Reactivada" } : prev);

            // 5. Notificar al técnico autónomo
            try {
                ...
                if (!targetTechUserId && trabajo.trabajador_id) {
                    try {
                        ...
                        if (foundTrab?.user_id) {
                            targetTechUserId = foundTrab.user_id;
                        }
                    } catch (_) { }
                }
                ...
                if (targetTechUserId) {
                    await createNotificacion({
                        ...
                        titulo: '✅ Cotización Reactivada',
                        mensaje: `El administrador general reactivó tu cotización para "${trabajo.sucursal || ''}".`,
                    ...
                    await createNotificacionByRole({
                        role: 'tecnico',
                        titulo: '✅ Cotización Reactivada',
                        mensaje: `El administrador general reactivó la cotización para "${trabajo.sucursal || ''}".`,
                        enlace: `/tecnico-autonomo/trabajo-detalle/${trabajo.id}?tab=cotizacion`
                    });
                }
            } catch (notiErr) {
            ...
            showAlert('Cotización Reactivada', 'La cotización anterior del técnico ha sido reactivada exitosamente.', 'success');
        } catch (error: any) {
            console.error("Error al reactivar cotización:", error);
            showAlert('Error', error.response?.data?.message || error.message || 'Hubo un problema al reactivar la cotización.', 'error');
        }
    };
    ...
    const handleClienteSolicitarRecotizacionTodas = async () => {
        ...
        try {
            ...
            const techUserId3 = (trabajo as any).tecnicoUserId || (trabajo as any).trabajador?.user_id;
            if (techUserId3) {
                try {
                    await createNotificacion({
                        user_id: techUserId3,
    ...
    const renderTaskCard = (tarea: SubTarea, isInteractive: boolean = true) => {
        ...
        const isVisita = trabajo?.tipo === "Visita";
        const isTecnico = (user?.role === 'tecnico' || user?.role === 'tecnico-normal' || user?.role === 'tecnico-autonomo');
                        ...
                        {(() => {
                            ...
                            return (
                                <button
                                    onClick={async (e) => {
                                        e.stopPropagation();
                                        if (trabajo?.id && trabajo.estado !== 'En Ejecución' && trabajo.estado !== 'Finalizado') {
                                            try {
                                                await updateTrabajo(trabajo.id, { estado: 'En Ejecución' });
                                                setTrabajo(prev => prev ? { ...prev, estado: 'En Ejecución' } : prev);
                                                localStorage.setItem(`trabajo_updated_${trabajo.id}`, Date.now().toString());
                                ...
                                >
                                    {isTaskReportDone ? '✏️ Editar Reporte' : '🛠️ Realizar Reporte / Empezar Trabajo'}
                                </button>
                            );
                        })()}
    ...
    return (
        <div className={styles.dashboardLayout}>
            ...
            <div className={styles.mainCard}>
                ...
                <div className={styles.scrollableContent}>
                    <div className={styles.contentWrapper}>
                        ...
                        {(() => {
                            ...
                            const steps = [
                                ...
                                { id: 4, label: "En Ejecución", icon: "🛠️" },
                                                ...
                                                <div style={{ minWidth: '120px' }}>
                                                    ...
                                                    <span style={{ fontSize: '14px', fontWeight: '800', color: '#0f172a', wordBreak: 'break-word' }}>
                                                        {(() => {
                                                            ...
                                                            return rawH.length > 5 ? rawH.substring(0, 5) : rawH;
                                                        })()}
                                                    </span>
                                                </div>
                                                <div style={{ minWidth: '120px' }}>
                                                ...
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                                    ...
                                                    {(subTareas.some(t => t.esCotizacion) || actualReporte) && (
                                                        <div style={{ background: '#fff', borderRadius: '24px', padding: '24px', boxShadow: '0 4px 24px rgba(0,0,0,0.06)', border: '1px solid #fde68a' }}>
                                                                                        ...
                                                                                        <>
                                                                                            ...
                                                                                            <div style={{ display: 'flex', gap: '8px', marginTop: '8px', flexWrap: 'wrap' }}>
                                                                                                ...
                                                                                                {tarea.quoteData && (user?.role === 'tecnico' || user?.role === 'autonomo') && trabajo?.estado !== 'Cotización Enviada' && cotizaciones.length === 0 && (
                                                                                                    <button
                                                                                                        onClick={() => {
                                                                                                            setQuoteToSend(tarea);
                                                                                                            setShowSendQuoteConfirmModal(true);
                                                                                                        }}
                                                                                                        style={{ flex: 1, padding: '10px 14px', background: 'linear-gradient(135deg, #10b981, #059669)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '12px', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', transition: 'all 0.2s', boxShadow: '0 4px 12px rgba(16, 185, 129, 0.2)' }}
                                                                                                        onMouseEnter={e => e.currentTarget.style.transform = 'translateY(-1px)'}
                                                                                                        onMouseLeave={e => e.currentTarget.style.transform = 'none'}
                                                                                                    >
                                                                                                        <HiOutlinePaperAirplane size={14} style={{ transform: 'rotate(45deg)' }} /> Enviar Cotización
                                                                                                    </button>
                                                                                                )}
                                                            ...
                                                            {/* Botones Aceptar/Re Cotizar para cotizaciones del BD (tecnico autonomo) */}
                                                            {cotizaciones.length > 0 && trabajo?.estado === 'Cotización Enviada' && !subTareas.some(t => t.esCotizacion) && (() => {
                                                                const canApprove = user?.role === 'autonomo' || user?.role === 'cliente' || user?.role === 'admin-autonomo' || user?.role === 'admin' || user?.role === 'administrador-general' || user?.role === 'gerente-general';
                                                                if (!canApprove) return null;
                                                                const cotizActual = cotizaciones[0];
                                                                const totalMonto = cotizActual?.monto ? Number(cotizActual.monto).toLocaleString('es-MX') : '---';
                                                                return (
                                                                    <div style={{ background: '#fafafa', border: '1.5px solid #f1f5f9', borderRadius: '14px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '16px' }}>
                                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                                            <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: '#f26522' }} />
                                                                            <span style={{ fontSize: '13px', fontWeight: '800', color: '#1e293b' }}>Cotización del Técnico — Pendiente de Aprobación</span>
                                                                        </div>
                                                                        <div style={{ background: 'linear-gradient(135deg, #fff7ed 0%, #fef3c7 100%)', border: '2px solid #f59e0b', borderRadius: '14px', padding: '14px 18px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                                            <span style={{ fontSize: '13px', fontWeight: '800', color: '#92400e', textTransform: 'uppercase' }}>Monto Total</span>
                                                                            <span style={{ fontSize: '22px', fontWeight: '900', color: '#f26522' }}>${totalMonto}</span>
                                                                        </div>
                                                                        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                                                            <button
                                                                                onClick={async () => {
                                                                                    try {
                                                                                        const targetTechName = trabajo?.tecnico || 'Jesus Escalante';
                                                                                        const targetTrabajadorId = trabajo?.trabajador_id || (trabajo as any)?.trabajador?.id || 1;
                                                                                        await updateTrabajo(trabajo!.id, {
                                                                                        ...
                                                                                        if (cotizActual?.id) { try { await updateCotizacion(cotizActual.id, { estado: 'Aprobada', monto: cotizActual.monto }); } catch (e) { } }
                                                                                        const targetTechUserAccept3 = (trabajo as any)?.tecnicoUserId || (trabajo as any)?.trabajador?.user_id || tecnicosData.find((t: any) => t.id === targetTrabajadorId)?.user_id;
                                                                                        if (targetTechUserAccept3) {
                                                                                            try {
                                                                                                await createNotificacion({
                                                                                                    user_id: targetTechUserAccept3,
                                                                                                    titulo: '🎉 Cotización Aceptada',
                                                                                        ...
                                                                                        setTrabajo(prev => prev ? { ...prev, estado: 'Cotización Aceptada', tecnico: targetTechName, trabajador_id: targetTrabajadorId } : prev);
                                                                                        showAlert('Cotización Aceptada', `Has aceptado la cotización por $${totalMonto}.`, 'success');
                                                                                    } catch (error) { showAlert('Error', 'Hubo un problema al actualizar el estado.', 'error'); }
                                                                                }}
                                                                                style={{ flex: 1, padding: '10px 14px', background: 'linear-gradient(135deg, #22c55e, #16a34a)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: '800', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}
                                                                            >
                                                                                ✓ Aceptar Cotización
                                                                            </button>
                                                                            <button
                    ...
                    {/* El Chat de Negociación ha sido eliminado por solicitud */}
                    {
                        activeTab === 'Trabajo' && (
                            <div>
                                {/* 📅 DÍA Y HORA DE EJECUCIÓN DEL TRABAJO (Lo primero que ve y define el técnico al aceptarse la cotización) */}
                                {['Cotización Aceptada', 'Cotización Aprobada', 'Aceptada', 'En Ejecución', 'En Proceso', 'Asignado'].includes(trabajo?.estado || '') && (
                                    <div style={{
                                        background: 'linear-gradient(135deg, #ecfdf5 0%, #d1fae5 100%)',
                                        border: '2px solid #10b981',
                                        borderRadius: '20px',
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                                <div style={{ background: '#10b981', color: 'white', borderRadius: '50%', width: '38px', height: '38px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '20px', fontWeight: 'bold' }}>
                                                    📅
                                                </div>
                                                <div>
                                                    <h4 style={{ margin: 0, fontSize: '15px', fontWeight: '850', color: '#065f46' }}>
                                                        Día y Hora de Ejecución del Trabajo
                                                    </h4>
                                                    <p style={{ margin: '2px 0 0 0', fontSize: '12px', color: '#047857' }}>
                                                        {(trabajo?.fecha_programada && (trabajo?.horaAsignada || trabajo?.hora_programada))
                                                            ? "Horario confirmado para la realización de este trabajo."
                                                            : "Por favor define el día y la hora en que realizarás este trabajo para notificar al Administrador y Cliente."}
                                                    </p>
                                                </div>
                                            </div>
                                            {(trabajo?.fecha_programada && (trabajo?.horaAsignada || trabajo?.hora_programada)) && !isEditingExecutionTime && (
                                                <span style={{
                                                    background: '#10b981',
                                                    color: '#ffffff',
                                                    padding: '6px 14px',
                                        ...
                                        {(isEditingExecutionTime || (!trabajo?.fecha_programada && !trabajo?.horaAsignada && !trabajo?.hora_programada)) ? (
                                            <div style={{ background: '#ffffff', padding: '18px 20px', borderRadius: '16px', border: '1.5px solid #10b981', display: 'flex', flexDirection: 'column', gap: '14px' }}>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
                                                    <span style={{ fontSize: '12px', fontWeight: '800', color: '#065f46', textTransform: 'uppercase', letterSpacing: '0.5px' }}>
                                                        Ingresar Día y Hora de Ejecución
                                                    </span>
                                                    <span style={{ fontSize: '11px', color: '#64748b' }}>
                                                        Se notificará automáticamente al Administrador General
                                                    </span>
                                                </div>
                                                ...
                                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '14px' }}>
                                                    <div>
                                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
                                                            <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#334155' }}>Día de Ejecución *</label>
                                                            <div style={{ display: 'flex', gap: '5px' }}>
                                                                <button
                                                                    type="button"
                                                                    onClick={() => setExecFecha(new Date().toISOString().split('T')[0])}
                                                                    style={{ fontSize: '11px', fontWeight: '600', padding: '2px 7px', borderRadius: '6px', border: '1px solid #cbd5e1', background: '#fff', color: '#475569', cursor: 'pointer' }}
                                                                >
                                                                    Hoy
                                                                </button>
                                                                <button
                                                    ...
                                                    <div>
                                                        <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#334155', marginBottom: '6px' }}>Hora de Ejecución *</label>
                                                        <select
                                                            value={execHora}
                                                            onChange={(e) => setExecHora(e.target.value)}
                                                            style={{
                                                ...
                                                <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end', marginTop: '4px' }}>
                                                    ...
                                                    <button
                                                        ...
                                                        onClick={handleSaveExecutionTime}
                                                        disabled={isSavingExecTime}
                                                        style={{ padding: '10px 22px', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: '800', cursor: 'pointer', boxShadow: '0 3px 10px rgba(16, 185, 129, 0.25)', opacity: isSavingExecTime ? 0.7 : 1 }}
                                                    >
                                                        {isSavingExecTime ? 'Guardando...' : '✓ Guardar y Confirmar Horario'}
                                                    </button>
                                                </div>
                                            ...
                                            <div style={{ display: 'flex', gap: '24px', background: 'white', padding: '14px 18px', borderRadius: '14px', border: '1px solid #a7f3d0', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between' }}>
                                                <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap', alignItems: 'center' }}>
                                                    <div>
                                                        <span style={{ fontSize: '11px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', display: 'block' }}>📅 Día de Ejecución</span>
                                                        <span style={{ fontSize: '14px', fontWeight: '800', color: '#0f172a' }}>
                                                            {trabajo?.fecha_programada ? (trabajo.fecha_programada.includes('-') ? trabajo.fecha_programada.split('-').reverse().join('/') : trabajo.fecha_programada) : 'Pendiente'}
                                                        </span>
                                                    </div>
                                                    <div>
                                                        <span style={{ fontSize: '11px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', display: 'block' }}>⏰ Hora de Ejecución</span>
                                                        <span style={{ fontSize: '14px', fontWeight: '800', color: '#0f172a' }}>
                                                    ...
                                                    <div>
                                                        ...
                                                        <span style={{ fontSize: '14px', fontWeight: '800', color: '#059669' }}>
                                                            {trabajo?.tecnico || 'Jesus Escalante'}
                                                        </span>
                                                    </div>
                                                </div>
                                                {((user?.role === 'tecnico' || user?.role === 'tecnico-normal' || user?.role === 'tecnico-autonomo') || user?.role === 'admin' || user?.role === 'autonomo') && (
                                                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
                                                        ...
                                                        {(user?.role === 'tecnico' || user?.role === 'tecnico-normal' || user?.role === 'tecnico-autonomo') && (
                                                            <button
                                                                type="button"
                                                                onClick={handleIniciarEjecucionTrabajo}
                                                                style={{
                                                                    padding: '8px 18px',
                                                                    background: trabajo?.estado === 'En Ejecución'
                                                                    ...
                                                                    fontWeight: '850',
                                                                    cursor: 'pointer',
                                                                    boxShadow: trabajo?.estado === 'En Ejecución'
                                                                        ? '0 4px 12px rgba(16, 185, 129, 0.3)'
                                                                        : '0 4px 12px rgba(242, 101, 34, 0.35)',
                                                                    display: 'flex',
                                                                    alignItems: 'center',
                                                                    gap: '6px',
                                                                    transition: 'all 0.2s ease'
                                                            ...
                                                            >
                                                                {trabajo?.estado === 'En Ejecución' ? '⚡ En Ejecución (Continuar)' : '🚀 Empezar Ejecución del Trabajo'}
                                                            </button>
                                                        )}
                                                    </div>
                                                )}
                                                ...
                                                const onSuccess = async (position: GeolocationPosition) => {
                                                    ...
                                                    try {
                                                        ...
                                                        if (trabajo) {
                                                            ...
                                                            await updateEstadoTrabajo(trabajo.id, {
                                                                estado: 'En Proceso', // Cambiado de 'En Espera' a 'En Proceso' porque el técnico ya llegó
                                                                hora_llegada: timeString,
                                                                latitud_llegada: lat,
                                                                longitud_llegada: lng
                                                            });
                                                            setTrabajo((prev: any) => prev ? { ...prev, estado: 'En Proceso', hora_llegada: timeString, latitud_llegada: lat, longitud_llegada: lng } : prev);
                                                        }
            ...
            {showSendQuoteConfirmModal && quoteToSend && (
                <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(15, 23, 42, 0.75)', backdropFilter: 'blur(8px)', zIndex: 99999, display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px' }}>
                    <div style={{ background: '#ffffff', borderRadius: '24px', padding: '32px', maxWidth: '460px', width: '100%', boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25)', textAlign: 'center' }}>
                        <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: '#ecfdf5', color: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 20px auto', fontSize: '32px' }}>
                            <HiOutlinePaperAirplane style={{ transform: 'rotate(45deg)' }} />
                        </div>
                        <h3 style={{ margin: '0 0 12px 0', fontSize: '20px', fontWeight: '800', color: '#0f172a' }}>¿Enviar Cotización?</h3>
                        <p style={{ margin: '0 0 20px 0', fontSize: '14px', color: '#64748b', lineHeight: '1.6' }}>
                            Una vez confirmada, la cotización se enviará al Administrador General para su revisión y aprobación.
                        </p>
                        <div style={{ display: 'flex', gap: '12px' }}>
                            <button
                                onClick={() => { setShowSendQuoteConfirmModal(false); setQuoteToSend(null); }}
                                style={{ flex: 1, padding: '13px', background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', borderRadius: '14px', fontWeight: '700', fontSize: '14px', cursor: 'pointer' }}
                            ...
                            <button
                                ...
                                style={{ flex: 1, padding: '13px', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', color: 'white', border: 'none', borderRadius: '14px', fontWeight: '800', fontSize: '14px', cursor: 'pointer', opacity: isSendingQuote ? 0.7 : 1, boxShadow: '0 8px 16px rgba(16, 185, 129, 0.2)' }}
                                disabled={isSendingQuote}
                            >
